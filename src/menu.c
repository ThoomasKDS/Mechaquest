...
int afficher_dialogue(game_t *game, joueur_t *j, SDL_Rect *sprite_p, SDL_Rect *pnj_sprite, img_pnj_t *sprite_pnj, img_player_t *sprite_playerH, char *pseudo, char *dialogue, int choix) {
    ...
    int largeurEcran, hauteurEcran;
    SDL_GetRendererOutputSize(game->renderer, &largeurEcran, &hauteurEcran);
    rectangle_t fondDialogue, textRect, pseudoRect, infoRect1, infoRect2;
    ...
    creer_rectangle(&infoRect1, 300, 30, (largeurEcran / 2) + 450, hauteurEcran - 40, 255, 255, 255, 255, "A pour continuer");
    creer_rectangle(&infoRect2, 300, 30, (largeurEcran / 2) + 450, hauteurEcran - 40, 255, 255, 255, 255, "choisir entre 1 et 3");
    ...
    while (index < len) {
        ...
        if (!choix) draw_rect(game, &infoRect1, draw_text_center);
        else draw_rect(game, &infoRect2, draw_text_center);
        SDL_RenderPresent(game->renderer);
        ...
        while (waitingForKey) {
            while (SDL_PollEvent(&event)) {
                ...
                if (!choix && event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_a) {
                    waitingForKey = false;
                    textIndex = 0;
                    memset(displayedText, 0, sizeof(displayedText));
                } else if (choix && event.type == SDL_KEYDOWN) {
